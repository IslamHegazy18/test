...
//            ));
                        ...
                        if ($value['quantity'] > 0) {
                            ...
                            if ($this->ReservationDetail->save($ReservationDetails_array)) {
                                ...
                                if (!empty($User_to['User']['token'])) {
                                    try {
                                        $this->push_notification_voucher($User_to['User']['token'], '', '', $User_to['User']['id'], "R");
                                    } catch (Exception $exc) {
        ...
        if (!empty($Username) && !empty($Password)) {
            ...
            if (!empty($checkUser)) {
                ...
                if ($this->Reservation->updateAll(
                    ...
                    $User_to = $this->User->find('first', array('conditions' => array('User.id' => $Service_Provider['Service_Provider']['user_id'])));
                    try {
                        $this->push_notification_voucher($User_to['User']['token'], '', '', $User_to['User']['id'], "RUR");
                    } catch (Exception $exc) {
                        //echo $exc->getTraceAsString();
                    }
        ...
        if (!empty($Username) && !empty($Password)) {
            ...
            if (!empty($checkUser)) {
                ...
                if ($this->Reservation->updateAll(
                    ...
                    try {
                        $this->push_notification_voucher($User_to['User']['token'], '', '', $User_to['User']['id'], "RUP");
                    } catch (Exception $exc) {
                        // echo $exc->getTraceAsString();
                    }
                }
        ...
        if (!empty($Username) && !empty($Password)) {
            ...
            if (!empty($checkUser)) {
                ...
                if ($this->Reservation->updateAll(
                    ...
                    try {
                        $this->push_notification_voucher($User_to['User']['token'], '', '', $User_to['User']['id'], "RSR");
                    } catch (Exception $exc) {
                        // echo $exc->getTraceAsString();
                    }
        ...
        if (!empty($Username) && !empty($Password)) {
            ...
            if (!empty($checkUser)) {
                ...
                if ($this->Reservation->updateAll(
                    ...
                    $User_to = $this->User->find('first', array('conditions' => array('User.id' => $Reservation['Reservation']['user_id'])));
                    try {
                        $this->push_notification_voucher($User_to['User']['token'], '', '', $User_to['User']['id'], "CSR");
                    } catch (Exception $exc) {
                        //echo $exc->getTraceAsString();
                    }
            ...
            if (!empty($checkUser)) {
                ...
                if ($ApprovalCode == $Reservation['Reservation']['approval_code']) {
                    ...
                    if ($voucher_res != 'failed') {
                        ...
                        $voucher_arr['Voucher']['amount'] = $amount;
                        ...
                        $voucher_arr['Voucher']['Voucher_OTP'] = $json_res['Response']['PaySvcRs']['MsgRqHdr']['CustomProperties']['CustomProperty'][3]['Value'];
                        ...
                        if ($this->Voucher->save($voucher_arr)) {
                            ...
                            if ($this->Reservation->updateAll(
                                ...
                                $this->push_notification_voucher($token, $voucher, $amount, $user_id, "V");
                                $statusFlag = TRUE;
                            }
                        }
                    }
    ...
    function push_notification_voucher($token, $voucher, $amount, $user_id, $type) {
        ...
        if ($type == 'V') {
            $messege = "تم تحويل مبلغ $amount من شركة أيوا نظير القيام بالخدمة, برجاء التوجه لأقرب منفذ فوري و صرف المبلغ, $voucher";
        } else if ($type == 'R') {
            $messege = "يوجد حجز جديد برجاء المراجعه";
            $navigation = "Settings";
        } else if ($type == 'RUR') {
            $messege = "لقد تم تغيير في الحجز من قبل المستخدم برجاء المراجعه";
            $navigation = "Settings";
        } else if ($type == 'RUP') {
            $messege = "لقد تمة عملية الدفع من قبل المستخدم برجاء المراجعه";
            $navigation = "Settings";
        } else if ($type == 'RSR') {
            $messege = "لقد تم تغيير في الحجز من قبل مقدم الخدمة برجاء المراجعه";
            $navigation = "ReservationUser";
        } else if ($type == 'CSR') {
            $messege = "لقد تم تأكيد الحجز من قبل مقدم الخدمة برجاء المراجعه";
            $navigation = "ReservationUser";
        } else if ($type == 'CancelU') {
            $messege = "لقد تم إلغاء الحجز من قبل المستخدم برجاء المراجعه";
            $navigation = "Settings";
        } else if ($type == 'CancelR') {
            $messege = "لقد تم إلغاء الحجز من قبل مقدم الخدمة برجاء المراجعه";
            $navigation = "ReservationUser";
        }
    ...
    public function generateVoucher($mobile = null, $amount = null) {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_PORT => "2021",
            CURLOPT_URL => "https://gw-int.fawrystaging.com:2021/CoreWeb/rest/ApplicationBusinessFacadeService",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_FOLLOWLOCATION => true,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_ENCODING => "",
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_POSTFIELDS => "{\r\n\"Request\":{\r\n\"SignonRq\":{\r\n\"ClientDt\":\"2011-02-17T23:27:52.905\",\r\n\"CustLangPref\":\"ar-eg\",\r\n\r\n\"SignonProfile\":{\r\n\"Sender\":\"Aiwa_INT\",\r\n\"MsgCode\":\"PmtAddRq\",\r\n\"Version\":\"V1.0\"\r\n}\r\n},\r\n\"PaySvcRq\":{\r\n\"RqUID\": \"6c5b28da-329f-4903-9ba9-82fcb97a972b\",\r\n\"MsgRqHdr\":{\r\n\"NetworkTrnInfo\":{\r\n\"OriginatorCode\":\"Aiwa\"\r\n},\r\n\"CustomProperties\":{\r\n\"CustomProperty\":[\r\n{\r\n\"Key\":\"Expiry_Days\",\r\n\"Value\":\"7\"\r\n}\r\n]\r\n}\r\n},\r\n\"PmtAddRq\":{\r\n\"PmtInfo\":[\r\n{\r\n\"BillingAcct\":\"$mobile\",\r\n\r\n\"BillTypeCode\":\"98778\",\r\n\"DeliveryMethod\":\"INT\",\r\n\"CurAmt\":{\r\n\"Amt\":\"$amount\",\r\n\"CurCode\":\"EGP\"\r\n},\r\n\"DepAccIdFrom\":{\r\n\"AcctId\":\"105266\",\r\n\"AcctType\":\"SDA\",\r\n\"SecureAcctKey\": \"gdyb21LQTcIANtvYMT7QVQ==\", \"AcctCur\":\"EGP\" },\r\n\r\n\"PmtMethod\":\"Cash\",\r\n\"PrcDt\":\"$PrcDt\"\r\n}\r\n]\r\n}\r\n}\r\n}\r\n}\r\n",
            CURLOPT_HTTPHEADER => array(
                "cache-control: no-cache",
                "content-type: application/json",
        ...
        if (!empty($Username) && !empty($Password)) {
            ...
            if (!empty($checkUser)) {
                ...
                if ($this->Reservation->updateAll(
                    ...
                    try {
                        $this->push_notification_voucher($User_to['User']['token'], '', '', $User_to['User']['id'], $type);
                    } catch (Exception $exc) {
                        //echo $exc->getTraceAsString();
                    }
                }
    ...
    public function get_Messages_serviceProvied() {
        ...
        if (!empty($Username) && !empty($Password)) {
            ...
            if (!empty($checkUser)) {
                $this->loadModel('Message');
                $Message_data = $this->Message->find('all', array('conditions' => array('Message.service_provider_id' => $checkUser['Service_Provider']['id'], 'Message.user_serviceprovider_flag' => '1'), "order" => array('Message.created DESC'), "group" => array('Message.user_id')));
                foreach ($Message_data as $key => $value) {
                    $Last_Message = $this->Message->find('first', array('conditions' => array('Message.service_provider_id' => $value['User']['id'], 'Message.service_provider_id' => $checkUser['Service_Provider']['id']), "order" => array('Message.created DESC')));
